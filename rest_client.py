    ...
    #    print('ACCESS token.....{}'.format(resp_dict['access_token']))
     ...
     def create_contact(self, row, access_token):
      try:
        ...
        endpoint = "{}contact/createOrUpdate/email/{}".format(cfg.appconfig['api_url'], row[7].strip())
        #print("Endpoint : {}".format(endpoint))
        headers = {}
  ...
  "properties": [
    ...
    },
    {
     ...
     def convertToTimestamp(self,dateAsStr, pattern):
        ...
        if dateAsStr == '..':
              return ''
        midnightDate =  datetime.combine(datetime.strptime(dateAsStr, pattern), datetime.min.time())
